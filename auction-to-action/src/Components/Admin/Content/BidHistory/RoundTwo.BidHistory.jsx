...
function RoundTwoBidHistory() {
  ...
  const fetchBidHistory = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${serverUrl}/api/admin/bid-history?round=2`,
        {
  ...
  const confirmDelete = async () => {
    try {
      ...
      await axios.delete(
        `${serverUrl}/api/admin/bid-history/${selectedBid._id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const saveEdit = async () => {
    try {
      ...
      const response = await axios.put(
        `${serverUrl}/api/admin/bid-history/${editingBid._id}`,
        {
            ...
            {selectedBid && (
              <VStack align="start" spacing={4}>
                ...
                <HStack>
                  ...
                  <Badge
                    colorScheme={
                      selectedBid.rewardType === "cash"
                        ? "green"
                        : selectedBid.rewardType === "resources"
                          ? "blue"
                          : selectedBid.rewardType === "challenge"
                            ? "orange"
                            : "gray"
                    }
                  >
                ...
                {selectedBid.deductionAmount > 0 && (
                  <HStack>
                    ...
                    <Text color="red.500" fontWeight="semibold">
                      ₹{selectedBid.deductionAmount}
                    </Text>
                  </HStack>
                )}
                {selectedBid.resourcesGained &&
                  Object.keys(selectedBid.resourcesGained).length > 0 && (
                    <VStack align="start">
                      <Text fontWeight="bold">Resources Gained:</Text>
                      <Box pl={4}>
                        {Object.entries(selectedBid.resourcesGained).map(
                          ([resource, amount]) => (
                            <Text key={resource}>
                              {resource}: {amount}
                            </Text>
              ...
              <VStack spacing={4}>
                <FormControl>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleEditChange("mysteryBoxReward", e.target.value)
                    }
                  />
                </FormControl>
                <FormControl>
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      handleEditChange("rewardType", e.target.value)
                    }
                  >
                ...
                <FormControl>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleEditChange("bidAmount", e.target.value)
                    }
                  />
                </FormControl>
                <FormControl>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleEditChange("cashReward", e.target.value)
                    }
                  />
                </FormControl>
                <FormControl>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleEditChange("deductionAmount", e.target.value)
                    }
                  />
                </FormControl>
                <FormControl>
                ...
                <FormControl>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleEditChange("teamCode", e.target.value)
                    }
                  />
                </FormControl>
              </VStack>
            ...
            <AlertDialogBody>
              ...
              {selectedBid && (
                <Box mt={2} p={2} bg="gray.100" borderRadius="md">
                  <Text>
                    <strong>Mystery Box:</strong> {selectedBid.mysteryBoxReward}
                  </Text>
                  <Text>
                    <strong>Amount:</strong> ₹{selectedBid.bidAmount}
                  </Text>
                  <Text>
                    <strong>Team:</strong> {selectedBid.teamName}
                  </Text>
                </Box>
              )}
            </AlertDialogBody>
