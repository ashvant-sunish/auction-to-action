  ...
  useEffect(() => {
    ...
    socket.on("wheelUpdate", (data) => {
      if (data.action === "itemSelected" && data.round === round) {
        ...
        if (
          currentSelectedBid &&
          data.selectedItem?.itemCode === currentSelectedBid.itemCode
        ) {
          setCurrentSelectedBid(null);
          setIsSelecting(false);
          setWheelStopped(false);
  ...
  const handleCloseSelection = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        const selectedIndex = availableItems.findIndex(
          (item) => item.id === currentSelectedBid.id,
        );
        if (selectedIndex !== -1) {
          const newAvailableItems = removeBidFromArray(
            availableItems,
            selectedIndex,
          );
      ...
      alert(
        `Failed to select item: ${error.response?.data?.message || error.message}`,
      );
      return;
    }
              ...
              >
                {availableItems.map((item, i) => (
                  <div
                    ...
                    className="card"
                    onClick={() =>
                      !isSelecting && alert(`Clicked ${item.title}`)
                    }
                    style={{
                          ...
                          >
                            ...
                            <div
                              style={{
                                ...
                                bottom: 0,
                                background:
                                  "linear-gradient(135deg, rgba(0,0,0,0.3) 0%, rgba(26,26,46,0.4) 50%, rgba(15,52,96,0.3) 100%)",
                                borderRadius: "12px",
                                margin: 0,
                                padding: 0,
                              }}
                            ...
                            >
                              ...
                              <div
                                style={{
                                  ...
                                  color: "#FFD700",
                                  textShadow:
                                    "2px 2px 4px rgba(0,0,0,0.9), 0 0 8px rgba(0,0,0,0.7)",
                                }}
                              >
