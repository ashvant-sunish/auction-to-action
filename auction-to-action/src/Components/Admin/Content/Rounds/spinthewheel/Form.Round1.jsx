...
function FormRound1() {
  ...
  const fetchGameState = async () => {
    try {
      ...
      if (response.data && response.data.currentRound === 1 && response.data.isAuctionLive) {
        setLiveGameState({
  ...
  return (
    <Box maxW="800px" mx="auto" p={6}>
      ...
      {wheelSelection && (
        <Card mb={6} bg={
          wheelSelection.status === 'CONFIRMED' ? 'green.50' :
          wheelSelection.status === 'SKIPPED' ? 'orange.50' : 'blue.50'
        } borderColor={
          wheelSelection.status === 'CONFIRMED' ? 'green.200' :
          wheelSelection.status === 'SKIPPED' ? 'orange.200' : 'blue.200'
        } borderWidth="2px">
          <CardBody>
            <Flex align="center" justify="space-between" mb={2}>
              <HStack>
                <Icon as={
                  wheelSelection.status === 'CONFIRMED' ? FaCheck :
                  wheelSelection.status === 'SKIPPED' ? MdSkipNext : FaRandom
                } color={
                  wheelSelection.status === 'CONFIRMED' ? 'green.600' :
                  wheelSelection.status === 'SKIPPED' ? 'orange.600' : 'blue.600'
                } />
                <Heading size="sm" color={
                  wheelSelection.status === 'CONFIRMED' ? 'green.600' :
                  wheelSelection.status === 'SKIPPED' ? 'orange.600' : 'blue.600'
                }>
                  {wheelSelection.status === 'CONFIRMED' ? '✅ Wheel: Item Confirmed' :
                   wheelSelection.status === 'SKIPPED' ? '⏭️ Wheel: Item Skipped' : '🎯 Wheel: Item Selected'}
                </Heading>
              </HStack>
              <Badge colorScheme={
                wheelSelection.status === 'CONFIRMED' ? 'green' :
                wheelSelection.status === 'SKIPPED' ? 'orange' : 'blue'
              } fontSize="xs">
            ...
            <Text fontSize="sm" color="gray.600" mb={2}>
              Bid #{wheelSelection.itemDetails?.bidNumber} | Code: {wheelSelection.itemDetails?.itemCode}
            </Text>
            {wheelSelection.itemDetails?.basePrice && (
              <Text fontSize="sm" color="green.600" mb={2}>
                Base Price: ₹{wheelSelection.itemDetails.basePrice.toLocaleString()}
              </Text>
            )}
            {wheelSelection.itemDetails?.resources && Object.keys(wheelSelection.itemDetails.resources).length > 0 && (
              <Box mt={2} p={2} bg="green.50" borderRadius="md" border="1px solid" borderColor="green.200">
                <Text fontSize="xs" color="green.700" fontWeight="semibold" mb={1}>
                  📦 Resources:
                </Text>
                <HStack spacing={2} flexWrap="wrap">
                  {Object.entries(wheelSelection.itemDetails.resources).map(([type, qty]) => (
            ...
            {wheelSelection.status === 'SELECTED' && autoFillEnabled && (
              <Button size="sm" mt={2} colorScheme="blue" onClick={autoFillFromWheelSelection}>
                Auto-Fill Form
              </Button>
            )}
          </CardBody>
      ...
      <Card mb={6} bg="gray.50" borderColor="gray.200" borderWidth="1px">
        <CardBody py={3}>
          <HStack justify="space-between">
            <Box>
              <Text fontWeight="semibold" fontSize="sm">Auto-Fill from Wheel Selections</Text>
              <Text fontSize="xs" color="gray.600">Automatically populate item details when wheel selects an item</Text>
            </Box>
            <Switch 
      ...
      <Alert status="info" mb={6}>
        ...
        <Box>
          ...
          <AlertDescription>
            Enter the details of bids won by teams in Round 1. Team information will be auto-fetched when you enter a valid team code.
            {wheelSelection && (
              <Text mt={2} color="blue.600" fontWeight="bold">
                🎯 Connected to spinning wheel! Item details can be auto-filled from wheel selections.
              </Text>
            )}
            {liveGameState && (
              <Text mt={2} color="red.600" fontWeight="bold">
                🔴 Live spinning wheel is active! New bids are being selected in real-time.
              </Text>
            )}
          </AlertDescription>
        </Box>
              ...
              <Card bg="green.50" borderColor="green.200" borderWidth="1px">
                <CardBody>
                  ...
                  <SimpleGrid columns={{ base: 2, md: 4 }} spacing={3}>
                    {Object.entries(itemResources).map(([resourceType, quantity]) => (
                      <Box key={resourceType} textAlign="center" p={2} bg="white" borderRadius="md" border="1px solid" borderColor="green.200">
                  ...
                  <Text fontSize="xs" color="gray.600" mt={3} textAlign="center">
                    These resources will be automatically added to the team's inventory when the trade is completed.
                  </Text>
                </CardBody>
              </Card>
            </Box>
          ...
          <FormControl isRequired maxW="300px">
            ...
            <NumberInput
              ...
              onChange={(valueString, valueNumber) => 
                setFormData(prev => ({ ...prev, bidAmount: valueNumber || 0 }))
              }
              min={0}
              step={100}
            >
